...
// Archive pagination: /blog/page-N.html, generated by generate-blog-pages.mjs.
// Lower priority than page 1 — these are navigation, not destinations.
function paginationUrls() {
  const readdirSync = _readdirSync;
  return readdirSync(new URL('../blog/', import.meta.url))
    .filter((f) => /^page-\d+\.html$/.test(f))
    .sort((a, b) => Number(a.match(/\d+/)[0]) - Number(b.match(/\d+/)[0]))
    .map((f) => ({ loc: `/blog/${f}`, lastmod: TODAY, changefreq: 'weekly', priority: '0.5' }));
}
...
const staticUrls = [
  ...
  // posts) would have been silently missing from the sitemap.
  ...paginationUrls(),
